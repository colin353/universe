...
pub fn extract_code(root_dir: &Path, output_filename: &str) {
    ...
    extract_from_dir(&root_dir, &mut builder);
    builder.finish();
}

fn extract_from_dir(root_dir: &Path, output: &mut sstable::SSTableBuilder<File>) {

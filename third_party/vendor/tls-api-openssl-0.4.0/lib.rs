...
impl tls_api::TlsConnectorBuilder for TlsConnectorBuilder {
    ...
    fn add_root_certificate(&mut self, cert: tls_api::Certificate) -> Result<&mut Self> {
        let cert = openssl::x509::X509::from_der(&cert.into_der()).map_err(Error::new)?;
    ...
    for TlsStream<S>
{
    fn shutdown(&mut self) -> io::Result<()> {
        match self.0.shutdown() {
            Ok(_) | Err(openssl::ssl::Error::ZeroReturn) => Ok(()),
    ...
    S: io::Read + io::Write + fmt::Debug + Send + Sync + 'static,
{
    match e {
        ...
        openssl::ssl::HandshakeError::Interrupted(s) => tls_api::HandshakeError::Interrupted(

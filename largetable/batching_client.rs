...
struct BatchingClient<T, C: largetable_client::LargeTableClient> {
    cache: VecDeque<BTreeMap<(String, String), T>>,
    client: C,
}
...
impl<T: protobuf::Message + Clone, C: largetable_client::LargeTableClient> BatchingClient<T, C> {
    ...
    fn add_cache(&mut self) {
        self.cache.push_back(BTreeMap::new());
    }
    ...
    fn perform_flush(&self) {
        let cache = match self.cache.front() {
            Some(x) => x,
            None => return,
        };
    ...
    fn write(&mut self, row: &str, col: &str, message: T) {
        if let Some(cache) = self.cache.back_mut() {
